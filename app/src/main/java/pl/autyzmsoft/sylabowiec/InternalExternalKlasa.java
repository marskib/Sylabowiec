...
public class InternalExternalKlasa extends Activity {
  ...
  protected void onCreate(@Nullable Bundle savedInstanceState) {
    ...
    setContentView(R.layout.activity_internal_externak_klasa);
  ...
  public void bInternalClick(View view) {
    ...
    //wywolania browsera:
    intent.putExtra(FileChooserActivity.INPUT_START_FOLDER,
        wewnetrznaSD); //ostatni parametr powinien byc, bo problemy...
...
*/
  ...
  public String[] getExternalStorageDirectoriesSkib() {
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) { //Method 1 for KitKat & above
      ...
      for (File file : externalDirs) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
          ...
          addPath = Environment.MEDIA_MOUNTED.equals(
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
      ...
      for (int i = 0; i < results.size(); i++) {
        if (!results.get(i).toLowerCase().contains("ext") && !results.get(
            i).toLowerCase().contains("sdcard")) {
          Log.d(LOG_TAG, results.get(i) + " might not be extSDcard");
          results.remove(i--);
        }
      }
